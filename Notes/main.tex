...
\item If $X$ is $(n \geq -1)$-connected, then $x=_{X}y$ is $(n-1)$-connected for all $x,y:X$ (see Exercise X)
\end{itemize}
...
\[
  \Bigl((z : S^1) \to B(z)\Bigr) \to \Bigl(x : B(\ast) \times (\Sloop_*(x)
  =^B_{\Sloop} x)\Bigr),
...
\paragraph{Exercise:} check that the the 
...
\end{lemma}
\begin{proof}
  Let $p : P(\star_{S^1})$. We define $f : (x : X) \to P(x)$ by $S^1$-elimination:
  \begin{align*}
    f(\star_{S^1}) &:= p \\
    \apd_f{\Sloop} &:= \text{`isProp'} : \ub{\trp^P_{\Sloop}(p) = p}{p =^P_{\Sloop} p} 
  \end{align*}
\end{proof}
...
\[
  ...
  \end{tikzcd}
\]
where $F:=\fib_f(\star_B)$ is the fiber of $f$ and the arrows are homomorphisms of groups until
