...
\begin{itemize}
\item \axel{Homotopy theory is the study of paths between points in topological spaces, and continuous transformations between these paths. Draw picture}
\item \axel{Turns out: types in type theory can be interpreted as spaces}
  \begin{itemize}
  \item $A : \UU$ `A is a type' ---> \axel{draw}
  \item $x:A$ ---> \axel{draw}
  \item $p:x=_Ay$ ---> \axel{draw}
  \item $p,q:x=_Ay$ ---> \axel{draw}
  \item Synthetic homotopy theory = study of identity types in (homotopy) type theory
  \end{itemize}
\end{itemize}

\begin{itemize}
  \item \axel{Particularly interesting: loops} -- homotopy groups. This is what we will study. Group structure by composing loops \axel{draw $\pi_1(X,x)$}
\end{itemize}


\section*{Paths}

\axel{Key tool: path induction}: Let $x:A$. In order to prove a statement/construct an element of type $(y:A) (p : x = y) \to B(x,p)$, it suffices to prove/construct an element of $B(x,\refl_x)$.
...
\begin{example}
  Given $f :X \to Y$ and $p:x=y$, we have $\ap_f(p) : f(x) = g(x)$
\end{example}
...
\axel{How to define concatenation of paths?}

\begin{example}
  Given $p: x = y$ and $q:y = z$, we can define $p\cdot q : x = z$ and $p^{-1} : y = z$ by path induction on $p$, setting $\refl_{x} \cdot q = q$ and $\refl_{x}^{-1} := \refl_{x}$ 
\end{example}
  Get
\begin{itemize}
...
Naive def of first homotopy group (incorrect!): $\pi_1(X,x) := (x = x)$.\\
%
Almost but not quite right -- too many proofs of the group axioms.
%
