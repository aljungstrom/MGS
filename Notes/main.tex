...
\subsection*{Why?}
\begin{itemize}
\item Understanding types better
  \begin{itemize}
  \item Types `=' spaces, so need to understand their higher structure
  \end{itemize}
\item Use type-theoretic tools to simpify problems from traditional homotopy theory/alg. topology
  \begin{itemize}
    \item E.g. compute homotopy groups of spheres
  \end{itemize}
\end{itemize}

\subsection*{Preliminary plan}
\begin{enumerate}
\item Preliminaries/defn of homotopy groups
\item Higher inductive types, $\pi_1(S^1)$
\item Long exact sequence of homotopy groups, $\pi_3(S^2)$
\item The Blakers-Massey theorem and applications: $\pi_n(S^n)$ and cohomology
\end{enumerate}

\subsection*{Notation/conventions}
\begin{itemize}
\item In general: follow HoTT book.
\item $\refl_{(x)} : x =_A x$
\item $\UU$ = the universe of types 
\item $\trp^{B}_p : B(x) \to B(y)$ where $B:A \to \UU$ and $p : x =_A y$
\item $(x: X) \to B(x)$ instead of $\Pi_{x:X}B(x)$
\item $(x : X) \times B(x)$ instead of $\Sigma_{x:X} B(x)$
\item `Path induction' instead of `J-rule' or `identity induction'
\end{itemize}
\axel{
Let $x_0 : X$ and $B: (x : X) \to x_0 = x \to \UU$. In order to construct elements $B(x,p)$ for all $x:X$ and $p : x_0 = x$, it suffices to do so when $x := x_0$ and $p := \refl_{x_0}$.
}
...
\begin{axiom}[Univalence]
  The map
  \[
    \trp^{Y \mapsto X \simeq Y}_{(-)}(\mathsf{idEquiv}_X) : X=_\UU Y \to (X \simeq Y)
  \]
  is an equivalence.
\end{axiom}
...
\axel{The thing that makes types complicated is their higher structure. The notion of $n$-type helps us stratify this.}
...
\end{definition}
\begin{itemize}
  \item Convention: `connected' = `$0$-connected'
\end{itemize}
...
\section*{Towards homotopy groups: path algebra}

\paragraph{Recall:}
Have $\_\cdot\_ : x = y \to y = z \to x = z$ and $(-)^{-1} : x = y \to y = x$ s.t.
\begin{itemize}
...
And
\begin{itemize}
\item Given $f :X \to Y$ and $p:x=y$, we have $\ap_f(p) : f(x) = g(x)$
\item Exercise $\ap_f(p\cdot q) = \ap_f(p) \cdot \ap_f(q)$
\end{itemize}

\paragraph{Q.} What if $f : (x:X) \to B(x)$ is a dependent function? $f(x) = f(y)$ doesn't type check!

\begin{definition}
  Given $B: X \to \UU$ dep. type, $p: x_0 = y_0$, and terms $b_0 : B(x_0)$ and $b:B(x_1)$, define
  \[(b_0 =^B_p b_1) := (\trp^B_p(b_0) = b_1) \]
  $\uparrow$ the type of \textbf{dependent paths} from $b_0$ to $b_1$ (over $p$). 
\end{definition}

\axel{Coincides with usual equality type when $p$ is $\refl$}

\begin{itemize}
\item Can now define dependent $\ap$ for $f: (x:X) \to B(x)$ , i.e.\ $\apd_f : (p : x = y) \to f(x) =^B_p f(y)$ by pattern-matching.
\end{itemize}
...
\begin{definition}
  ...
  \begin{itemize}
    \item Multiplication: $\trunc{p} \cdot \trunc{q} = \trunc{p \cdot q}$
  \end{itemize}
\end{definition}
